...
class FrequencyCountSortedUnsorted {
    ...
    public static void FrequencyUnsorted(int arr[]) {
        ...
        // ! create new array with size of maximum element - (java already initialize all elements of array to zero)
        int arr1[] = new int[max + 1];

        // ! count frequency
        for (int ele : arr) arr1[ele]++;
        for (int i = 0; i <= max; i++)
            if (arr1[i] != 0) System.out.println(i + " -> " + arr1[i]);
    }
    ...
    public static void main(String[] args) {
        ...
        //*This method will Work On Only Sorted Array
        Arrays.sort(arr); 
